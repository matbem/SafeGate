...
export const api = {
  ...
  getLogs: async (sinceTimestamp: string): Promise<GetLogsResponse> => {
    const response = await axios.get<GetLogsResponse>(`${API_URL}/api/v1/admin/logs`, {
  ...
  addUser: async (users: UserData[]): Promise<UserOperationsResponse> => {
    const response = await axios.post<UserOperationsResponse>(`${API_URL}/api/v1/admin/add_user`, {
      users_list: users
    });
    return response.data;
  },
  ...
  deleteUsers: async (ids: number[]): Promise<UserOperationsResponse> => {
    const response = await axios.delete<UserOperationsResponse>(`${API_URL}/api/v1/admin/users`, {
