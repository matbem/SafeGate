...
const AdminPage: React.FC = () => {
  ...
  // ZMIANA: Tutaj musi być User[], ponieważ api.getUsers() zwraca pełne obiekty użytkowników
  ...
  const fetchData = async () => {
    try {
      ...
      setUsers(usersData); // Teraz typy się zgadzają (User[])
    } catch (err) {
      ...
      <nav className="bg-white shadow px-6 py-4 flex justify-between items-center">
        ...
        <div className="flex gap-4">
          <button
            ...
            className={`flex items-center gap-2 ${activeTab === 'LOGS' ? 'text-blue-600 font-bold' : 'text-gray-600'}`}
          >
            <FileText size={18} /> Logi Dostępu
          </button>
          <button
            onClick={() => setActiveTab('USERS')}
            className={`flex items-center gap-2 ${activeTab === 'USERS' ? 'text-blue-600 font-bold' : 'text-gray-600'}`}
          >
